...
```mermaid
flowchart TD
    S1["阶段一：上下文锚定<br/>.cursorrules · .sprint · 模块文档"]
    S2["阶段二：架构师蓝图<br/>实施蓝图 · 安全边界标注"]
    S3["阶段三：设计师方案<br/>视觉方案 · 样式 + 动画"]
    S4["阶段四：执行 + 提交<br/>模块化执行 · 原子 Commit"]
...
```mermaid
flowchart TD
    S1["1. 确认上下文<br/>.cursorrules + .sprint + docs/modules/"] --> S2
    S2["2. 架构师蓝图 (02)<br/>安全边界 + 分步计划"] --> Q
    Q{"涉及 UI?"}
    Q -- "是" --> S3["3. 设计师方案 (03)<br/>视觉参数 + 样式代码"]
    Q -- "否" --> S4
    S3 --> S4["4. 逐步执行 (04)<br/>每步 1~2 文件 · @引用"]
    S4 --> S5["5. 验证 + Commit (05)<br/>Diff 安全审查"]
    S5 --> S6{"全部 Step 完成?"}
    S6 -- "否：清空上下文" --> S4
    S6 -- "是" --> S7["集成测试 → 合并主分支"]
```
