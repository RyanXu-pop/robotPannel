...
```mermaid
flowchart TD
    IN["输入：架构师蓝图 Step N<br/>+ 设计师视觉方案（如有）"] --> S1
    S1["1. 构造 Prompt<br/>蓝图 + 方案 + @引用"] --> S2
    S2["2. AI 生成 Diff"] --> S3
    S3{"3. Diff 安全审查"}
    S3 -->|"通过"| S4["4. 验证 + Commit"]
    S3 -->|"侵入冻结区"| R1["拒绝 Diff → 重新构造 Prompt"]
    R1 --> S1
    S4 --> S5{"验证通过?"}
    S5 -->|"是"| S6["5. 清空上下文 → Step N+1"]
    S5 -->|"小问题"| FIX["同会话修复 → 重新验证"]
    FIX --> S4
    S5 -->|"蓝图不可行"| ESC["回退到架构师<br/>修改蓝图后重新执行"]
```
